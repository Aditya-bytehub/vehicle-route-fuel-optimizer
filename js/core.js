(() => {
  const KEYS={users:'routeforge_users',current:'routeforge_current_user',routes:'routeforge_routes',vehicles:'routeforge_vehicles',prefs:'routeforge_preferences',draft:'routeforge_route_draft'};
  const LEGACY_KEYS={users:'routex_users',current:'routex_current_user',routes:'routex_routes',vehicles:'routex_vehicles',prefs:'routex_preferences',draft:'routex_route_draft'};
  function migrateLegacyStorage(){
    Object.keys(KEYS).forEach(name=>{
      const next=KEYS[name], legacy=LEGACY_KEYS[name];
  ...
  function seed(){
    let us=users();
    const demoUsers=[
      {id:'user_demo',name:'Aditya Sharma',email:'demo@routeforge.app',phone:'9876543210',password:'demo1234',role:'user',status:'active',createdAt:'2026-08-12'},
      {id:'user_anmol',name:'Anmol Sehgal',email:'anmol@routeforge.app',phone:'9876500001',password:'demo1234',role:'user',status:'active',createdAt:'2026-08-13'},
      {id:'user_riya',name:'Riya Mehta',email:'riya@routeforge.app',phone:'9876500002',password:'demo1234',role:'user',status:'active',createdAt:'2026-08-14'},
      {id:'user_kunal',name:'Kunal Verma',email:'kunal@routeforge.app',phone:'9876500003',password:'demo1234',role:'user',status:'active',createdAt:'2026-08-15'}
    ];
    // Ensure every demo user exists even when localStorage already contains one or more users.
    demoUsers.forEach(d=>{
      const idx=us.findIndex(u=>u.id===d.id || u.email?.toLowerCase()===d.email.toLowerCase());
      if(idx===-1) us.push(d);
      else us[idx]={...d,...us[idx],id:d.id,email:d.email,role:d.role};
    });
    const admin={id:'admin_demo',name:'RouteForge Admin',email:'admin@routeforge.app',phone:'9999999999',password:'admin1234',role:'admin',status:'active',createdAt:'2026-08-10'};
    const adminIndex=us.findIndex(u=>u.email?.toLowerCase()==='admin@routeforge.app' || u.id==='admin_demo');
  ...
  function demoRoutes(){
    ...
    write(KEYS.routes,[
      ...
      mk('route_demo4','user_demo','Panipat Morning Dispatch',.002),
      mk('route_demo5','user_demo','Urgent North Route',-.002),
      mk('route_demo2','user_anmol','Morning Express',.004),
      mk('route_demo3','user_riya','City Collection',-.003)
    ]);
  }
  ...
  window.RouteForge={...(window.RouteForge||{}),KEYS,uid,read,write,users,routes,vehicles,userRoutes,userVehicles,seed,current,setCurrent,logout,requireUser,saveUser,toast,esc,fmt,go,pageChrome};
  seed();
})();
