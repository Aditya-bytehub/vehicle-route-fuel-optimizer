document.addEventListener("DOMContentLoaded", () => {
  ...
  f.addEventListener("submit", (ev) => {
    ...
    const name = (document.getElementById("name")?.value || "New User").trim(),
    ...
    if (!name || !email || !pw)
      return (
        (e.textContent = "Please complete all required fields."),
    ...
    if (RouteForge.users().some((u) => u.email.toLowerCase() === email))
      return (
        (e.textContent = "An account with this email already exists."),
        (e.style.display = "block")
      );
    const u = {
      id: RouteForge.uid("user"),
      name,
      email,
      phone,
      password: pw,
    ...
    RouteForge.write(RouteForge.KEYS.users, [...RouteForge.users(), u]);
